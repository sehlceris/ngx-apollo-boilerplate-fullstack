...
export abstract class AbstractTokenGuard extends AbstractTemplateGuard {
  ...
  protected abstract async getJwtPayloadFromContext(context: ExecutionContext): Promise<JwtSingleUseUserPayload>;

  protected async checkCanActivate(context: ExecutionContext): Promise<boolean> {
    const jwtPayloadType: JwtPayloadType = this._reflector.get<JwtPayloadType>('jwtPayloadType', context.getHandler());
